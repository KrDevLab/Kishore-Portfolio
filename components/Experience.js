function Experience() {
  ...
  return (
    <section id="experience" className="py-24 relative" data-name="experience" data-file="components/Experience.js">
      <div className="container mx-auto px-6 max-w-4xl">
        ...
        <div className="relative border-l border-white/10 ml-4 md:ml-0 md:pl-0">
          {timeline.map((item, idx) => (
            <div key={idx} className="mb-12 ml-8 md:ml-0 md:grid md:grid-cols-2 items-center reveal relative group">
              
              {/* Timeline Dot */}
              <div className="absolute -left-10 md:left-1/2 md:-ml-2.5 top-0 md:top-1/2 md:-mt-2.5 w-5 h-5 rounded-full bg-[#0F172A] border-2 border-[var(--accent-start)] z-10 group-hover:bg-[var(--accent-start)] group-hover:shadow-[0_0_15px_rgba(56,189,248,0.5)] transition-all"></div>
              
              {/* Desktop Center Line Override */}
              <div className="hidden md:block absolute left-1/2 top-0 bottom-[-3rem] w-px bg-white/10 -ml-px"></div>

              {/* Content Box */}
              <div className={`glass-card p-6 ${idx % 2 === 0 ? 'md:mr-12 md:text-right' : 'md:col-start-2 md:ml-12'} hover:border-white/20 transition-colors`}>
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mb-3 ${
                  item.type === 'active' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 
                  item.type === 'learning' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 
                  'bg-white/5 text-gray-400'
                }`}>
                  {item.date}
                </span>
                <h3 className="text-xl font-bold mb-1 text-white">{item.role}</h3>
                <h4 className="text-[var(--accent-start)] text-sm font-medium mb-4">{item.company}</h4>
